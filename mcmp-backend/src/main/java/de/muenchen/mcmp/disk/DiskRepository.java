...
public interface DiskRepository extends JpaRepository<Disk, Long> {
    ...
    SELECT d.*
    FROM cmp.server s
    join cmp.disk d on s.id = d.server_id
    WHERE s.id = :serverId
    AND (
        :isAdmin
        OR :isReadonly
...
""", nativeQuery = true)
    List<Disk> findByServerId(@Param("serverId") Long serverId,
    ...
    @Query(value = """
        SELECT disk.*
        FROM cmp.disk
        JOIN cmp.server ON disk.server_id = server.id
        WHERE server.cloud_id = :cloudId
    """, nativeQuery = true)
    List<Disk> findByServerCloudId(@Param("cloudId") Long cloudId);
}
