...
// TestQuickDiscovery validates the QuickDiscovery method functionality for triggering discovery operations.
// This test ensures proper handling of quick discovery requests and validates response scenarios.
//
// Test scenarios covered:
// - Valid request with IP address returning status 200
// - Invalid request with domain name returning status 400
func TestQuickDiscovery(t *testing.T) {
	// Define test cases for quick discovery operations
	tests := []struct {
		name          string                // Descriptive name for the test case
		request       QuickDiscoveryRequest // Input quick discovery request
		httpResponse  *http.Response        // Mock HTTP response to return
		httpError     error                 // Mock HTTP error to return
		expectedError error                 // Expected error condition
	}{
		{
			// Test case 1: Valid request with IP address returning status 200
			name: "valid request with IP address - status 200",
			request: QuickDiscoveryRequest{
				CallbackURL: "https://localhost/callback/quickdiscovery/55",
				DiscoveryIP: "192.168.192.5",
			},
			httpResponse: &http.Response{
				StatusCode: http.StatusOK,
		...
		{
			// Test case 2: Invalid request with domain name returning status 400
			name: "invalid request with domain name - status 400",
			request: QuickDiscoveryRequest{
				CallbackURL: "https://localhost/callback/quickdiscovery/56",
				DiscoveryIP: "www.example.org",
			},
			httpResponse: &http.Response{
				StatusCode: http.StatusBadRequest,
				Body:       io.NopCloser(strings.NewReader(`{"error": "invalid IP address"}`)),
			},
			httpError:     nil,
			expectedError: errors.New("failed to post quick discovery request: HTTP status code: 400, body: {\"error\": \"invalid IP address\"}"),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockHttpClient := &MockHttpClient{
				DoFunc: func(req *http.Request) (*http.Response, error) {
					...
					if req.Body != nil {
						...
						var quickDiscoveryRequest QuickDiscoveryRequest
						err = json.Unmarshal(bodyBytes, &quickDiscoveryRequest)
						if err != nil {
							t.Errorf("failed to unmarshal request body: %v", err)
						}

						// Validate the request body fields
						if quickDiscoveryRequest.CallbackURL != tt.request.CallbackURL {
							t.Errorf("expected callbackUrl '%s', got '%s'", tt.request.CallbackURL, quickDiscoveryRequest.CallbackURL)
						}
						if quickDiscoveryRequest.DiscoveryIP != tt.request.DiscoveryIP {
							t.Errorf("expected discovery_ip '%s', got '%s'", tt.request.DiscoveryIP, quickDiscoveryRequest.DiscoveryIP)
						}
					}
					return tt.httpResponse, tt.httpError
				},
			...
			client := &Client{
				httpClient:        mockHttpClient,
				urlQuickDiscovery: "https://localhost/cmdb/quickdiscovery",
				DebugLogger:       logging.NewDebugLogger(nil),
				debug:             false,
			}

			// Execute the method under test with the specified request
			err := client.QuickDiscovery(tt.request)
...
// TestPostTag validates the PostTag method functionality for tagging configuration items.
// This test ensures proper handling of tag creation requests and validates response scenarios.
//
// Test scenarios covered:
// - Valid request returning status 200 with successful tag creation
// - Invalid request returning status 400 when tag already exists
// - Request body validation to ensure correct payload is sent
func TestPostTag(t *testing.T) {
	// Define test cases for tag posting operations
	tests := []struct {
		name          string         // Descriptive name for the test case
		tag           string         // Input tag parameter
		ciSysId       string         // Configuration Item System ID
		httpResponse  *http.Response // Mock HTTP response to return
		httpError     error          // Mock HTTP error to return
		expectedError error          // Expected error condition
	}{
		{
			// Test case 1: Valid request returning status 200
			name:    "successful tag creation - status 200",
			tag:     "SNSVC0000002",
			ciSysId: "cd13985e9e44f214fa84ebc2dd623611",
			httpResponse: &http.Response{
				StatusCode: http.StatusOK,
				Body: io.NopCloser(strings.NewReader(`{
					"sys_id":"398770e5c3b87610d05cf6fe05013113",
					"key":"serviceid",
					"value":"SNSVC0000002",
					"ci":"cd13985e9e44f214fa84ebc2dd623611"
				}`)),
			},
			httpError:     nil,
			expectedError: nil,
		...
		{
			// Test case 2: Invalid request returning status 400 - tag already exists
			name:    "tag already exists - status 400",
			tag:     "SNSVC0000003",
			ciSysId: "2932d3a0ab2cf6102913e8f5bc4f0f91",
			httpResponse: &http.Response{
				StatusCode: http.StatusBadRequest,
				Body: io.NopCloser(strings.NewReader(`{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create mock HTTP client for this specific test case with request body validation
			mockHttpClient := &MockHttpClient{
				DoFunc: func(req *http.Request) (*http.Response, error) {
					// Validate request body
					if req.Body != nil {
						...
						// Parse the request body
						var tagRequest TagPostRequest
						err = json.Unmarshal(bodyBytes, &tagRequest)
						if err != nil {
							t.Errorf("failed to unmarshal request body: %v", err)
						}

						// Validate the request body fields
						if tagRequest.Key != "serviceid" {
							t.Errorf("expected key 'serviceid', got '%s'", tagRequest.Key)
						}
						if tagRequest.CI != tt.ciSysId {
							t.Errorf("expected ci '%s', got '%s'", tt.ciSysId, tagRequest.CI)
						}
					}
			...
			client := &Client{
				httpClient:  mockHttpClient,
				urlTag:      "https://localhost/tag/",
				DebugLogger: logging.NewDebugLogger(nil),
				debug:       false,
			}

			// Execute the method under test with the specified parameters
			err := client.PostTag(tt.tag, tt.ciSysId)
