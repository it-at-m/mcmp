...
func NewClient(config ClientConfig) (*Client, error) {
	...
	tlsConfig := &tls.Config{
		MinVersion:         tls.VersionTLS12, // Updated to TLS 1.3 for enhanced security
...
func (c *Client) SendPatchnightData(ctx context.Context, endpoint string, jsonData []byte) error {
	...
	// Debug logging for response details when debug mode is enabled
	c.DebugPrintf("Response Status: %s", resp.Status)
	c.DebugPrintf("Response Body: %s", string(body))
