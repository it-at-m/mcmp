...
import (
	...
	"github.com/it-at-m/mcmp/mcmp-eai-snow-repo-discovery/pkg/client/source"
	"github.com/it-at-m/mcmp/mcmp-eai-snow-repo-discovery/pkg/processor"
)

const (
	appName = "mcmp-eai-snow-repo-discovery"
)
...
type Config struct {
	...
	MCMP       []mcmp.Config
	SERVICENOW struct {
		snow.ClientConfig   `mapstructure:",squash"`
		WaitBetweenRequests string `mapstructure:"WaitBetweenRequests"`
	}
}
...
func run(ctx context.Context) error {
	...
	if cfg.SERVICENOW.ApiEndpoint != "" {
		//nolint:contextcheck // snow.NewClient does not accept context in common package
		snowClient, err = snow.NewClient(cfg.SERVICENOW.ClientConfig)
		if err != nil {
			logger.Warn("Failed to initialize ServiceNow client, CI reconciliation will be skipped", "error", err)
		}
		if cfg.SERVICENOW.WaitBetweenRequests != "" {
			waitDuration, err = time.ParseDuration(cfg.SERVICENOW.WaitBetweenRequests)
			if err != nil {
				logger.Error("Failed to parse WaitBetweenRequests duration, falling back to default (2s)", "error", err)
	...
	for _, rCfg := range cfg.REPO {
		...
		dataProcessor := processor.NewProcessor(repoClient, snowClient, logger)
		dataProcessor.SetWaitDuration(waitDuration)
