...
// {appName}.toml (e.g., mcmp-eai-rightsizing.toml).
const (
	appName = "mcmp-eai-rightsizing"
)
...
func run(ctx context.Context) error {
	...
	// Initialize MCMP Client (rightsizing-specific client)
	commonConfig := cfg.MCMP.ToClientConfig()
	...
	dataProcessor, err := processor.NewProcessor(mcmpClient, logger, cfg.PROCESSOR)
	if err != nil {
		return fmt.Errorf("failed to create processor: %w", err)
	}

	// Create a data source for rightsizing analysis
	// Using "mcmp" as hostname identifier
	sources := []app.DataSource[*mcmp.Rightsizing]{
		source.NewRightsizingSource(
			"mcmp",                       // Hostname identifier
			true,                         // Enabled
			dataProcessor,                // Data processor
			mcmpClient,                   // MCMP client
			cfg.MCMP.RightsizingEndpoint, // API endpoint
			logger,
		),
	}
	...
	logger.Info("Starting rightsizing analysis",
		...
		"min_samples", cfg.PROCESSOR.MinSampleSize,
		"cpu_high_load_threshold", cfg.PROCESSOR.CPUHighLoadThreshold,
		"memory_high_laod_threshold", cfg.PROCESSOR.MemoryHighLoadThreshold)
...
//
// Example Return Cases:
//   - "MCMP configuration invalid: missing API endpoint"
//   - "MCMP configuration invalid: invalid API URL format"
//   - "MCMP configuration invalid: missing authentication credentials"
//
// Integration:
//   - Called early in run() to fail fast on configuration errors
//   - Prevents initialization of expensive resources with invalid config
//   - Provides clear error messages for configuration correction
//
// Best Practices:
//   - Extend this method when adding new configuration sections
